...
from app.services.db_helpers import log_agent, save_provenance, save_risk_flag
from app.config import settings

# ── FinBERT NER pipeline (load once at module level) ─────
# ProsusAI/finbert — fine-tuned on financial text
# Falls back to CPU if no GPU available
_finbert_ner = None

def get_finbert():
    global _finbert_ner
    if _finbert_ner is None:
        model_name = "ProsusAI/finbert"
        tokenizer = AutoTokenizer.from_pretrained(model_name)
        model = AutoModelForTokenClassification.from_pretrained(model_name)
        device = 0 if torch.cuda.is_available() else -1
        _finbert_ner = hf_pipeline(
            "ner",
            model=model,
            tokenizer=tokenizer,
...
def extract_tables(pdf_path: str) -> list[dict]:
    """Extract tables from a PDF file path. Returns list of DataFrames as dicts."""
    results = []
    try:
...
def run_finbert_ner(text: str) -> list[dict]:
    """Run FinBERT NER on text chunk. Returns entity list."""
    try:
        ner = get_finbert()
        # FinBERT max tokens = 512; chunk text
...
def embed_and_store(app_id: str, doc_type: str, source: str, pages: list[dict]):
    """Chunk pages, embed, store in ChromaDB."""
    try:
        ...
        upsert_chunks(app_id, doc_type, source, all_chunks, embeddings)
